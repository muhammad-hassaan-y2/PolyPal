...
export default function LessonPage() {
    ...
    return (
        <div className="min-h-screen bg-[#FFFBE8]">
            ...
            >
                <h1 className="text-4xl font-bold text-[#FF9000] mb-12 text-center">
                ...
                {topics.map((topic, topicIndex) => (
                    <motion.div 
                        ...
                        className="mb-16"
                    >
                        <h2 className="text-3xl font-semibold text-[#FF7A00] mb-6">{topic.title}</h2>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                            {topic.items.map((item, itemIndex) => (
                                <Link key={itemIndex} href={item.href}>
                                    ...
                                    >
                                        <Card className="h-full overflow-hidden bg-white bg-opacity-80 backdrop-blur-lg border-2 border-[#FFE0B2] hover:border-[#FF9000] transition-all duration-300">
                                            <CardHeader className="flex flex-col items-center text-center p-6">
                                                ...
                                                >
                                                    <item.icon className="w-10 h-10 text-[#FF5500]" />
                                                </motion.div>
                                                <CardTitle className="text-2xl font-bold text-[#FF7A00] mb-2">{item.name}</CardTitle>
                                                <CardDescription className="text-[#FF5500] text-lg">{item.description}</CardDescription>
                                            </CardHeader>
                                        </Card>
                                    </motion.div>
                                </Link>
